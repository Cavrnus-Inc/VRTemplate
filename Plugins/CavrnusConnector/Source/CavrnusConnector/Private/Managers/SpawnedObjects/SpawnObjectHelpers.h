...
public:
	 ...
	 *
	 * This method spawns an actor of the specified class in the given world and sets it up
	 * using the provided object data. Gets transform from journal property.
	 *
	 * @param World The world in which to spawn the actor.
	 * @param ActorClass The class of the actor to spawn.
	...
	/**
	 * @brief Spawns an object actor with a provided transform and sets it up.
	 *
	 * This method spawns an actor of the specified class in the given world with the provided
	 * transform and sets it up using the provided object data. Used for local spawn calls.
	 *
	 * @param World The world in which to spawn the actor.
	 * @param ActorClass The class of the actor to spawn.
	 * @param SpawnTransform The transform to spawn the actor at.
	 * @param SpawnedObject The object data to set up the spawned actor.
	 * @param CollisionHandling The collision handling method for spawning.
	 * @return A pointer to the newly spawned actor.
	 */
	AActor* SpawnObjectAndSetupWithTransform(
		UWorld* World, 
	...
	/**
	 * @brief Sets up Cavrnus components on a deferred actor (before FinishSpawningActor).
	 * 
	 * This method should be called on a deferred actor to set up Cavrnus components
	 * before finishing the spawn. This ensures components are ready when BeginPlay() is called.
	 * 
	 * @param InActor The deferred actor to set up
	 * @param SpawnedObject The object data to use for setup
	 */
	void SetupCavrnusActorDeferred(AActor* InActor, const FCavrnusSpawnedObject& SpawnedObject);
