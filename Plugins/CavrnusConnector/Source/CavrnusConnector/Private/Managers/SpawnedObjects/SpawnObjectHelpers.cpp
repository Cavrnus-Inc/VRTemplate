...
AActor* SpawnObjectHelpers::SpawnObjectAndSetupWithTransform(
	UWorld* World, 
	...
	AActor* Owner)
{
	AActor* SpawnedActor = World->SpawnActorDeferred<AActor>(ActorClass, SpawnTransform, Owner, nullptr, CollisionHandling);
...
void SpawnObjectHelpers::SetupCavrnusActorDeferred(AActor* InActor, const FCavrnusSpawnedObject& SpawnedObject)
{
	// Set up the components on the deferred actor
	HandleSpawnedObject(InActor, SpawnedObject);
}
